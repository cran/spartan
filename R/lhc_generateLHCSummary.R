...
function(FILEPATH,PARAMETERS,MEASURES,LHC_ALL_SIM_RESULTS_FILE,LHCSUMMARYFILENAME,SPARTAN_PARAMETER_FILE=NULL,TIMEPOINTS=NULL,TIMEPOINTSCALE=NULL)
{
	...
	if(is.null(TIMEPOINTS))
	{
		if(file.exists(FILEPATH))
		{
			...
			{
				# WITH PREVIOUS SPARTAN, WE READ IN THE SPARTAN PARAMETER FILE AND COMBINED THIS WITH THE RESULT
				# HERE, THE PARAMETERS MAY BE IN THE RESULT FILE, BUT MAY NOT ALWAYS BE. THUS WE PROVIDE FUNCTIONALITY TO
				# READ THE PARAMETER VALUES IN IF NECESSARY
				if(is.null(SPARTAN_PARAMETER_FILE))
				{
				...
				{
					# READ THESE FROM THE LHC PARAMETER FILE GENERATED BY SPARTAN
					if(file.exists(paste(FILEPATH,"/",SPARTAN_PARAMETER_FILE,sep="")))
					{
						LHCTABLE<-read.csv(paste(FILEPATH,"/",SPARTAN_PARAMETER_FILE,sep=""),header=TRUE)
						SIM_PARAMS<- t(as.numeric(LHCTABLE[row,]))
						STRING_SIM_PARAMS<-paste(SIM_PARAMS,collapse=" ")
					}
					else
					{
						print("LHC Parameter file does not exist. Check variable SPARTAN_PARAMETER_FILE")
					}
				}
