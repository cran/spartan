...
function(FILEPATH,LHCSAMPLE_RESULTFILENAME,NUMSAMPLES,MEASURES,RESULTFILEFORMAT,RESULTFILENAME,TIMESTEP)
{
	...
	# BUT FIRST, WE MAY NEED TO CHANGE THE PARAMETER AND MEASURE STRINGS. WHEN R IMPORTS THE SPREADSHEET, ANY HYPHENS ARE CHANGED
	...
	for(SAMPLE in 1:NUMSAMPLES)
	{
		if(file.exists(paste(FILEPATH,"/",SAMPLE,"/",LHCSAMPLE_RESULTFILENAME,SAMPLE,".csv",sep="")))
		{
			...
			NL_RESULT<-read.csv(paste(FILEPATH,"/",SAMPLE,"/",LHCSAMPLE_RESULTFILENAME,SAMPLE,".csv",sep=""),sep=",",skip=6)
			...
			PARAM_RESULTFILEPATH = paste(FILEPATH,"/",SAMPLE,"/",RESULTFILENAME,sep="")
